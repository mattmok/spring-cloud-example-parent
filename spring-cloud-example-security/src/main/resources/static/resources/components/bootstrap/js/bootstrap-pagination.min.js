...
(function($){
	var BootstrapPaginator = function(container,options){
		this.selector = options.selector;
		this.href = options.href;
		this.$container = $($("tbody", container)[0] || container);
		this.emptydata = "<tr><td class='text-center' style='height:200px;line-height:200px;' colspan='"+$("thead > tr",container).children().length+"'>未找到相关数据</td></tr>";
		this.source = this.$container.html();
		this.render = template.get(options.templateId);
		if(this.render === undefined && this.source){
			this.render = template.compile('{{each data.list}}' + this.source + "{{/each}}");
		}
	...
	BootstrapPaginator.prototype = {
		constructor : BootstrapPaginator,
		_init: function(params){
			var that = this;
			$.post(this.href + "/1", params, function(data){
				that.$container.empty();
				that.$container.append(that.render(data)||that.emptydata);
				if(pagination = $(that.selector).data('twbs-pagination')){
					pagination.destroy();
				}
				var pages = data.data.pages;
				if(pages > 0){
					$(that.selector).twbsPagination({
						totalPages : pages,
						first: "&lt;&lt;",
						prev:"&lt;",
						next:"&gt;",
						...
						onPageClick : function(event, page) {
							$.getJSON(that.href + "/" + page, params, function(data){
								that.$container.empty();
