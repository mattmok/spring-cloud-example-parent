...
(function(){
    ...
    var MarkerClusterer =
        ...
        BMapLib.MarkerClusterer = function(map, options){
            ...
            this._labelName = opts["labelName"];
	...
	MarkerClusterer.prototype._createClusters = function(){
		...
		if (overlays[overlays.length-1].constructor == BMapLib.TextIconOverlay) {
			overlays[overlays.length-1].addEventListener("mouseover", function(event){
				var bdary = new BMap.Boundary();
				var textName = this._text.name;
				bdary.get("郑州市"+textName, function(rs){       //获取行政区域
					var count = rs.boundaries.length; //行政区域的点有多少个
					var pointArray = [];
					for (var i = 0; i < count; i++) {
						var ply = new BMap.Polygon(rs.boundaries[i], {strokeWeight: 3, strokeColor: "#38E1FF", fillColor: ""});
						ply.name = textName;
						thatMap.addOverlay(ply);  //添加覆盖物
						pointArray = pointArray.concat(ply.getPath());
					}
				});
				
				overlays[overlays.length-1].addEventListener("mouseout", function(event){
					var overlays = thatMap.getOverlays();
					for (var i = 0; i < overlays.length -1; i++){
						if(overlays[i].name == textName){
							thatMap.removeOverlay(overlays[i]);
						}
					}
				});
			});
...
//        var index = indexOf(marker, this._markers);
    ...
    /**
     * 获取单个聚合物的名称。
     * @return {String} 单个聚合物的名称。
     */
    MarkerClusterer.prototype.getLabelName = function() {
        return this._labelName;
    };

    /**
     * 设置单个聚合物的名称。
     * @param {String} 单个聚合物的名称。
     * @return 无返回值。
     */
    MarkerClusterer.prototype.setLabelName = function(labelName) {
    ...
    function Cluster(markerClusterer){
        ...
        this._labelName = markerClusterer.getLabelName();
        this._labels = [];
        this._func = markerClusterer._func;
        this._clusterMarker = new BMapLib.TextIconOverlay(this._center, {name:this._labelName,value : this._markers.length}, {"styles":this._markerClusterer.getStyles()});
...
//        var content = '<p style="padding:0px 13px;text-align:center;margin-bottom:0px;margin-top:5px;">'+marker.name+'</p></br><p style="padding:0px 13px;text-align:center;">'+'当前车辆:'+marker.vehicles+'辆'+'</p>';
        var content = '<p style="padding:0px 13px;text-align:center;margin-top:8px;">'+marker.name+': '+marker.vehicles+'辆'+'</p>';
        label.setContent(content);
        label.setPosition(position);
        var backgroundColor = this._styles[0].backgroundColor;
        var func = this._func;
        label.parkInfo = marker;
        label.addEventListener("mouseover", function(event){
        	label.setStyle({
            	height : '30px',
                color : "#fff",
                backgroundColor : '#f3b215',
                border : 'none',
                borderRadius : "25px",
                fontWeight : 'bold',
                lineHeight : "12px",
    ...
    Cluster.prototype.updateClusterMarker = function () {
        ...
        this._clusterMarker.setText({name : this._labelName, value : this._markers.length});
        ...
        this._clusterMarker.addEventListener("click", function(event){
			...
			for(var i = 0; i < overlays.length; i++){
				if (typeof overlays[i] == "object" && overlays[i].constructor != BMapLib.TextIconOverlay && overlays[i].constructor != BMap.Marker) {
					thatMap.removeOverlay(overlays[i]);
				}
			}
        });
